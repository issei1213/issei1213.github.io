...
![](/img/posts/compound-patter/usecase-1.png)  
実際に使用しているコンポーネントは以下  
``modal.tsx``
```tsx
...
![](/img/posts/compound-patter/usecase-2.png)
...
- 親コンポーネント→子コンポーネントや、孫コンポーネントに`props`を渡したい場合、複雑になる。
  - Ex: 
    - `React.Children`や`React.cloneElement`を使う場合
<details>
<summary>`React.Children`や`React.cloneElement`を使う場合 </summary>
...
```
    ...
    - `createContext`などを用いた`GlobalStore`を使う場合
---
## 感想
